// components/run/LiveStatsBar.tsx
"use client";
...
const LiveStatsBar: React.FC<LiveStatsBarProps> = ({ activeDuration, route, gpsSignalLost }) => {
  const t = useTranslations('LiveStatsBar');

  const totalDistance = route.reduce((acc: number, point: GeoPoint, index: number, arr: GeoPoint[]) => {
    if (index === 0) return 0;
    ...
    <div className="bg-neutral-800/80 backdrop-blur-md text-neutral-100 p-3 shadow-lg sticky top-0 z-30">
      <div className="container mx-auto flex justify-around text-center">
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="text-xl font-semibold tabular-nums">{totalDistance.toFixed(2)} <span className="text-sm font-normal">{t('km')}</span></div>
        </div>
        <div className="flex flex-col items-center">
          <div className="flex items-center gap-1 text-xs uppercase text-neutral-400 mb-0.5">
            <SpeedIcon /> {t('pace')}
          ...
          <div className="text-xl font-semibold tabular-nums">
            {pace > 0 ? pace.toFixed(2) : '--'} {pace > 0 ? <span className="text-sm font-normal">{t('minKm')}</span> : ''}
          </div>
        </div>
         <div className="flex flex-col items-center">
