...
import LeafletResizeFix from './LeafletResizeFix';            // ← 🆕 1️⃣
...
const SummaryPageClient: React.FC<SummaryPageClientProps> = ({ runId }) => {
  ...
  useEffect(() => {
    if (mapRef.current && runData && runData.route.length > 0) {
      const bounds = L.latLngBounds(runData.route.map((p) => [p.lat, p.lng]));
      if (bounds.isValid()) {
        mapRef.current.fitBounds(bounds, { padding: [50, 50] });
      } else if (runData.route.length === 1) {
        mapRef.current.setView(
          [runData.route[0].lat, runData.route[0].lng],
          15
        );
      }
    }
  ...
  const polylinePositions = runData.route.map(
    (p: GeoPoint) => [p.lat, p.lng] as [number, number]
  );
  const avgPace =
    runData.distance > 0 ? runData.duration / 60 / runData.distance : 0;
    ...
    <div className="container mx-auto px-4 py-8">
      <header className="mb-8 text-center">
        ...
        <p className="text-neutral-600">
          {t('yourRun')} - {formatDate(new Date(runData.createdAt), locale)}
        </p>
      </header>
        ...
        <div className="bg-white p-5 rounded-xl shadow-lg">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between py-2 border-b border-neutral-100">
              ...
              <span className="text-neutral-900 font-semibold">
                {avgPace > 0 ? `${avgPace.toFixed(2)} min/km` : '--'}
              </span>
            </div>
            <div className="flex justify-between py-2">
              ...
              <span className="text-neutral-900 font-semibold">
                {formatDate(new Date(runData.startTime), locale)}
              </span>
            </div>
          </div>
          ...
          <div className="mt-6 pt-5 border-t border-neutral-200">
            ...
            <p className="text-neutral-500 italic bg-neutral-50 p-4 rounded-md text-sm">
      ...
      {/* Ensure Leaflet fills its container */}
      <style jsx global>{`
        .leaflet-container {
          height: 100%;
          width: 100%;
